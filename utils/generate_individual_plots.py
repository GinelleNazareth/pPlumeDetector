...
# Script parameters for ping360_20230131_175845.bin
...
if __name__ == "__main__":
    ...
    #ax_polar.set_yticks([10,20], labels = ['10m', '20m']) # For AUV data
    #ax_polar.set_yticks([10, 20,30,40,50], labels=['10m', '20m', '30m', '40m', '50m']) # For AUV data
    ax_polar.set_yticks([10, 20, 30, 40, 50], labels=['1m', '2m', '3m', '4m', '5m'], color='white') #For JH data
    ...
    for label in radial_labels:
        ...
        label.set_size(10)  # For JH data
        label.set_color('black') # For JH data

    ax_polar.set_rlabel_position(270) # For JH data
    ...
    plt.setp(ax_polar.spines.values(), color='black', linewidth = 0.3, alpha=0.5)
    image_name = "Gridlines Black "
    plt.savefig(os.path.join(img_save_path, image_name), dpi=400, transparent=True)

    ax_polar.grid(True, color='white', linewidth = 0.3, alpha=0.0)
    plt.setp(ax_polar.spines.values(), color='white', alpha=0.0)
    ...
    for index, (timestamp, decoded_message) in enumerate(log.parser()):
        ...
        if angle == 399:
            ...
            ax.imshow(image, interpolation='none', cmap='binary',vmin=0, vmax=1)
