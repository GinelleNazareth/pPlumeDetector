...
import os
import pyproj
from src.block_cluster import block_cluster_fn
import timeit
...
class PPlumeDetector():
    ...
    def cluster(self):
        ...
        #TEST_CODE = '''clustered_cores_img, cluster_regions_img, labelled_regions_img, labelled_clustered_img, num_clusters = block_cluster_fn(seg_img, window_width_pixels, row_padding, col_padding, cluster_min_pixels)'''
        #SETUP = '''from pPlumeDetector.src.block_cluster import block_cluster_fn'''
        #self.clustering_time_secs = timeit.timeit(stmt=TEST_CODE, setup=SETUP, number=10)
        self.clustered_cores_img, self.cluster_regions_img, self.labelled_regions_img, self.labelled_clustered_img, self.num_clusters = block_cluster_fn(
            self.seg_img, self.window_width_pixels, row_padding, col_padding, self.cluster_min_pixels)
